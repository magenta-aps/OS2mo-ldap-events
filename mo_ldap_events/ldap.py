...
def configure_ad_connection(settings: Settings, client_strategy=ASYNC_STREAM) -> Connection:
...
def setup_persistent_search(context: Context, callback: Callable, search_parameters: dict):
    connection = context["user_context"]["ad_async_connection"]
    ...
    def ad_listener(event: dict) -> None:
        ...
        if event.get("type") == "searchResDone":
            print("Got SearchResultsDone in Persistent Search - Persistent Search is not supported by LDAP server, falling back to polling search")
            setup_poller(context, callback, search_parameters, now)
...
def _poller(connection: Connection, search_parameters: dict, callback: Callable, init_search_time: datetime) -> None:
    """
    ...
    while True:
        time.sleep(5)
        search_parameters = set_search_params_modify_timestamp(search_parameters, last_search_time)
...
def setup_poller(context: Context, callback: Callable, search_parameters: dict, init_search_time: datetime = None):
...
def set_search_params_modify_timestamp(search_parameters: Dict, timestamp: datetime):
    changed_str = "(modifyTimestamp>=" + timestamp.strftime("%Y%m%d%H%M%S") + "." + str(int(timestamp.microsecond / 1000)) + (timestamp.strftime("%z") or '-0000') + ")"
    print(changed_str)
    return {
        **search_parameters,
